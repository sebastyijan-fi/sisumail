...
|---|------|------|-------|-----|---------|
| 1 | MX | `<u>.sisumail.fi` | `10 <u>.v6.sisumail.fi.` | 300 | Tier 1 primary (blind relay) |
| 2 | MX | `<u>.sisumail.fi` | `20 spool.sisumail.fi.` | 300 | Tier 2 fallback (encrypted spool) |
| 3 | AAAA | `<u>.v6.sisumail.fi` | `<ipv6>` | 300 | Tier 1 destination (unique per user, served dynamically) |
...
- **Separate hostnames**: `<u>.v6.sisumail.fi` and `spool.sisumail.fi` have different TLS certificate holders — the user device and the relay, respectively.
...
- **Delegation**: see `docs/dns-delegation.md`
